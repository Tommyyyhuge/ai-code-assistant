...
| difficulty | SMALLINT | NOT NULL, CHECK(1-10) | 1 | 难度1-10 |
...
#####  `knowledge_problem_associations` (知识题目关联
...
- **分页规范**: `page` (页码, 开, `per_page` (每页数量, 默认20, 最00)
...
|------|------|------|------|
| page | integer | | 页码，默 |
| per_page | integer | | 每页数量，默0 |
| difficulty_min | integer | | 最小难1-10 |
| difficulty_max | integer | | 最大难1-10 |
...
| prompt_level | integer | | 请求的提示级1-3 |
...
2. **JWT 策略**: Access Token 有效1 小时，Refresh Token 有效7 3. **API 限流**: 基于 Redis 的滑动窗口限流，认证接口 5分钟，评测提10分钟
4. **CORS 配置**: 仅允许前端域名访5. **SQL 注入防护**: 使用 SQLAlchemy ORM，禁止原SQL
6. **XSS 防护**: 前端 React 自动转义，Markdown 渲染使用 DOMPurify
7. **AI API Key 加密**: 用户自定API Key 使用 AES-256-GCM 加密存储
...
4. **图片/文件**: 使用 CDN 或对象存储，数据库只URL
5. **分页**: 所有列表接口默20 页，最100 
### 4.3 Docker 部署说明
...
> 本文档为技术规格说明书 V1.0，基PRD V1.1 编制 
> 后续根据技术评审意见进行修订
### 3.3 ǰ״̬Zustand Store ƣ
